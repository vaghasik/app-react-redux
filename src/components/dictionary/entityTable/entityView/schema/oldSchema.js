...
// const options = [
// { value: "Lexico", label: "All tables" },
// { value: "App", label: "Application" },
// { value: "Service", label: "Service" },
// { value: "Entity", label: "Local" }
// ];
const options = [
...
class Schema extends Component {
  ...
  versionCommentInfoModalFunc(index) {
    const schemaFieldName = this.props.schemaData[
      this.props.schemaData.length - 1
    ].schemaInfo[index].columnName;
    this.setState({
  ...
  closeVersiondetailsModal() {
    this.props.schemaData.reverse();
    this.setState({ showVersionDetailModal: false });
  }
  downLoadPdf = () => {
    exportCSVFile(
      "",
      this.props.schemaData[this.props.schemaData.length - 1].schemaInfo,
      this.props.entityTitle,
      this.props.schemaData[this.props.schemaData.length - 1].version
    );
  };
  downLoadPdf2 = () => {
    exportCSVFile(
      "",
      this.props.schemaData[this.props.schemaData.length - 2].schemaInfo,
      this.props.entityTitle,
      this.props.schemaData[this.props.schemaData.length - 2].version
    );
  };
  downLoadPdf3 = () => {
    exportCSVFile(
      "",
      this.props.schemaData[this.props.schemaData.length - 3].schemaInfo,
      this.props.entityTitle,
      this.props.schemaData[this.props.schemaData.length - 3].version
    );
  };
  editCommentFunc(index, editColumnName) {
    //this.props.schemaData[this.props.schemaData.length - 1].schemaInfo[index].isEditButtonClicked = true;
    const schemaCommentData = this.props.schemaData[
      this.props.schemaData.length - 1
    ...
    this.setState({
      // editedDataObject,
      allDescriptionList: this.props.schemaData[
        this.props.schemaData.length - 1
      ].schemaInfo,
      schemaDataIndex: index,
      schemaCurrentUpdateComment: schemaCommentData,
  ...
  handleCommentChange(id, columnName, columnType, PL, event) {
    ...
    if (
      this.props.schemaData[this.props.schemaData.length - 1].schemaInfo[id]
        .comments[id] &&
      PL
    ) {
      this.props.schemaData[this.props.schemaData.length - 1].schemaInfo[
    ...
    this.state.UpdatedCommentsList.filter(a => a.columnName == columnName)
      .length > 0
      ? null
      : this.state.UpdatedCommentsList.push(editedRecordIds);
    console.log(
      "SchemaDetails",
      this.props.schemaData[this.props.schemaData.length - 1].schemaInfo
    );
    this.setState(
      {
        ...
        UpdatedCommentsList: this.state.UpdatedCommentsList,
        columnName: columnName,
        editedColumnType: columnType,
        commentsEditPl: PL,
        updatedCommentList: event.target.value,
        showPreviewBtn: true,
        isModifed: true,
      ...
      () => {
        ...
        console.log("editedRecordIds", this.state.UpdatedCommentsList);
      }
    );
  }
  stopCommentEdit() {
  ...
  handleCheckComment = (event, id, columnName) => {
    ...
    this.setState(prevState=>{
      let { UpdatedCommentsList, isAllSelected } = prevState;
      if(name === "selcetAllComment"){
        isAllSelected = valueChecked;
        UpdatedCommentsList = UpdatedCommentsList.map((item,index) => ({ ...item, currentChecked:valueChecked}));
      }else{
        UpdatedCommentsList = UpdatedCommentsList.map(item =>
          item.columnName === columnName ? { ...item, currentChecked: valueChecked } : item
        );
        isAllSelected = UpdatedCommentsList.every(item => item.currentChecked === true);
        console.log("dasdsa",isAllSelected)
      }
      return { isAllSelected, UpdatedCommentsList};
       },()=>{console.log(this.state.UpdatedCommentsList)});
       
  };
 /*  handleCheckComment = (event, id, columnName) => {
    console.log("selected data", this.state.UpdatedCommentsList);
    const target = event.target;
    const name = target.name;
    const valueChecked =
      target.type === "checkbox" ? target.checked : target.value;
    this.state.listChecked = [];
    //console.log("value", valueChecked);
    //this.setState({["checkedComent"+id]: valueChecked });
    let checkedComments = "";
    if (valueChecked) {
      checkedComments = this.state.UpdatedCommentsList.map(item =>
        item.columnName === columnName ? { ...item, isChecked: checked } : item
      );
    } else {
      checkedComments = this.state.UpdatedCommentsList.map((a, i) =>
        a.splice(i, 1)
      );
    }
    console.log("dasdasdasddsaddasdasd", checkedComments);
    this.setState(
      {
        ["checkedComent" + id]: valueChecked,
  ...
  handleSelectPropogationChange(propogationSelected, id, columnName) {
    this.props.schemaData[this.props.schemaData.length - 1].schemaInfo[id].PL =
      propogationSelected.label;
    console.log("propogationSelectedValue", propogationSelected.label);
    this.setState({ ["propogationSelectedValue" + id]: propogationSelected });
  }
  discardCommentChanges = () => {
    //console.log("editedRecordId", this.state.editedRecordId);
    this.setState({
      UpdatedCommentsList: [],
      editedRecordId: [],
      showEditCommentModal: false,
  ...
  saveCommentChanges = () => {
    ...
    console.log(this.props.schemaData);
    let filteredData = this.state.allDescriptionList.map(item => {
      return {
        columnName: item.columnName ? item.columnName:null,
        columnType: item.columnType ? item.columnType:null,
        PL: item.PL ? item.PL:null,
        comment: item.comment ? item.comment: item.comments.length > 0 ? item.comments.map(subItem =>subItem.comment):""
      };
    });
    this.setState(
      {
  ...
  render() {
    ...
    return (
      <div className="schema">
        {this.props.schemaData.length > 0 ? (
          <div className="schema-details">
            <div className="schema-virsio-download">
              {this.props.schemaData.length > 1 ? (
                ...
                >
                  <span>V{this.props.schemaData.length}</span>
                </span>
              ) : (
                ""
              )}
                          ...
                          <td className="schema-fields-data">
                            ...
                            {data.comments && data.comments.length > 0 ? (
                              data.comments[data.comments.length - 1].comment
                                .toLowerCase()
                                .indexOf("PII") !== -1 ? (
                                <div className="schema-col-name font-red">
                                  {" "}
                                  {data.columnName}{" "}
                                </div>
                              ) : data.comments[
                                ...
                                data.comments[data.comments.length - 1].comment
                                  .toLowerCase()
                                  .indexOf("encode") !== -1 ? (
                                <div className="schema-col-name font-blue">
                                  {" "}
                                  {data.columnName}
                                </div>
                              ) : (
                          ...
                          <td>
                            ...
                            <span className="schema-icons">
                              ...
                              <FontAwesomeIcon
                                ...
                                className="schema-icon"
                                onClick={() =>
                                  this.editCommentFunc(id, data.columnName)
                                }
                              />
                            </span>
                          </td>
                ...
                <Modal.Header closeButton>
                  <Modal.Title>
                    ...
                    <span className="total-version">
                      {" "}
                      (total &nbsp; {this.props.schemaData.length}){" "}
                    </span>{" "}
                  </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                              ...
                              <tr key={i}>
                                <td>
                                  ...
                                  {this.props.schemaData[
                                    ...
                                    <span>
                                      {this.props.schemaData[
                                        this.props.schemaData.length - 2
                                      ].version === subitem.version ? (
                                        <FontAwesomeIcon
                                          icon={faArrowCircleDown}
                                          className="schema-dowmload-icon"
 ...
 <div className="tbl-to-col"><table> */}
                      <tbody>
                        {this.props.schemaData.slice(0, 3).map((subitem, i) => {
                          return (
                            <tr key={i}>
                              {subitem.schemaInfo
                                .sort((a, b) =>
            ...
            {this.state.showEditCommentModal ? (
              ...
              >
                <Modal.Header closeButton>
                  <Modal.Title>Save the changes</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                  <table>
                    <thead>
                    ...
                    <tbody>
                      {this.state.UpdatedCommentsList.map((data, id) => {
                        console.log(
                          "test>>>>1",
                          data.comments &&
                            data.comments.map(subdata => subdata.pl)
                        );
                        return (
                          <tr>
                            ...
                            <td className="comment-check">
                              <input
                                ...
                                type="checkbox"
                                checked={data.currentChecked ? data.currentChecked :false}
                                onChange={event =>
